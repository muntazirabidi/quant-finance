...
if __name__ == "__main__":
    ...
    pairs = [
        # ETF Pairs - Highly liquid with strong fundamental relationships
        ('XLE', 'USO'),    # Energy sector ETF vs Oil ETF - Direct oil price exposure
        ('GDX', 'GLD'),    # Gold miners vs Gold - Miners leverage to gold prices
        ('XLF', 'VFH'),    # Financial sector ETFs - Similar bank stock compositions
        ('QQQ', 'SPY'),    # Tech-heavy vs Broad market - Tech sector dominance play

        # Consumer Staples - Stable businesses with similar market dynamics
        ('KO', 'PEP'),     # Coca-Cola vs PepsiCo - Beverage industry leaders
        ('WMT', 'TGT'),    # Walmart vs Target - Retail giants with similar models
        ('COST', 'WMT'),   # Costco vs Walmart - Wholesale/retail competition
        ('PG', 'CL'),      # P&G vs Colgate - Consumer products leaders

        # Financial Services - Shared regulatory and interest rate exposure
        ('JPM', 'BAC'),    # JPMorgan vs Bank of America - Leading US banks
        ('GS', 'MS'),      # Goldman Sachs vs Morgan Stanley - Investment banking
        ('BLK', 'BEN'),    # BlackRock vs Franklin - Asset management focus

        # Technology Hardware - Semiconductor industry dynamics
        ('INTC', 'AMD'),   # Intel vs AMD - CPU market competitors
        ('NVDA', 'AMD'),   # Nvidia vs AMD - GPU market competition
        ('NVDA', 'TSM'),   # Nvidia vs TSMC - GPU and semiconductor market dynamics
        ('TSM', 'UMC'),    # TSMC vs United Micro - Semiconductor foundries competition

        # Energy Companies - Integrated oil majors
        ('CVX', 'XOM'),    # Chevron vs Exxon - Oil major pair
        ('PSX', 'VLO'),    # Phillips 66 vs Valero - Refining focus

        # Payment Processing - Digital payments ecosystem
        ('V', 'MA'),       # Visa vs Mastercard - Card network duopoly
        ('PYPL', 'SQ'),    # PayPal vs Block - Fintech leaders
        
        # Healthcare - Pharmaceutical and insurance
        ('JNJ', 'PFE'),    # Johnson & Johnson vs Pfizer - Pharma giants
        ('MRK', 'LLY'),    # Merck vs Eli Lilly - Pharmaceutical research and products
        ('UNH', 'CVS'),    # UnitedHealth Group vs CVS Health - Healthcare services and pharmacy benefits management

        # Airlines - Travel industry dynamics
        ('AAL', 'UAL'),    # American vs United - Legacy carriers
        ('DAL', 'LUV'),    # Delta vs Southwest - Different business models in the airline industry

        # Automakers - Transition to electric vehicles
        ('TSLA', 'GM'),    # Tesla vs General Motors - Electric vehicle leadership vs traditional automotive
        ('F', 'TM'),       # Ford vs Toyota - Traditional vs hybrid and emerging electric vehicle markets

        # Retail - Online vs Traditional
        ('AMZN', 'WMT'),   # Amazon vs Walmart - Online retail giant vs traditional retail leader
        ('EBAY', 'ETSY'),  # eBay vs Etsy - Different approaches to online marketplaces

        # Technology - Software and Services
        ('MSFT', 'GOOGL'), # Microsoft vs Alphabet - Software and services giants with diverse tech portfolios
        ('CRM', 'ORCL'),   # Salesforce vs Oracle - Cloud computing and customer relationship management

        # Consumer Electronics - Market leaders
        ('AAPL', 'SNE'),   # Apple vs Sony - Tech innovation leaders with different product focuses

        # Telecommunications - Infrastructure and Services
        ('T', 'VZ'),       # AT&T vs Verizon - Major telecom service providers in the US
        ('TMUS', 'VOD'),   # T-Mobile US vs Vodafone - Wireless services with international presence

        # Real Estate - REITs with different focuses
        ('SPG', 'MAC'),    # Simon Property Group vs Macerich - Mall REITs with different property portfolios
    ]
    ...
    # Sort by Sharpe ratio to get the best risk-adjusted performers
    strong_performers = strong_performers.sort_values('sharpe_ratio', ascending=False)

    # Display top performers
    print("Top performing pairs based on risk-adjusted returns:")
    print(strong_performers[['pair', 'annualized_return', 'sharpe_ratio', 'win_rate', 'profit_factor', 'max_drawdown']].head())
